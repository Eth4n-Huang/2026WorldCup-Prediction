...
from step5c_devset    import dc_probs_for_matches, dc_top_scores_for_matches, dc_side_markets_for_matches
...
def generate_day_predictions(
    ...
    dc_ou25_arr, dc_btts_arr = dc_side_markets_for_matches(dc, upcoming)
    ...
    for i, (_, row) in enumerate(upcoming.iterrows()):
        ...
        rows.append({
            ...
            "dc_ou25":            round(float(dc_ou25_arr[i]), 4),
            "dc_btts":            round(float(dc_btts_arr[i]), 4),
        })
